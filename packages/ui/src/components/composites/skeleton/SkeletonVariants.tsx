...
export const CardSkeleton = ({ className }: { className?: string }) => (
  <div className={cn("glass flex flex-col gap-4 rounded-xl p-5 shadow-soft", className)}>
    <div className="flex items-center gap-3">
      <Skeleton variant="circular" width={40} height={40} />
      <div className="flex flex-1 flex-col gap-2">
        <Skeleton variant="text" width="60%" height={14} />
        <Skeleton variant="text" width="40%" height={10} className="opacity-60" />
      </div>
    </div>
    <Skeleton variant="rectangular" height={160} className="w-full rounded-lg" />
  ...
  <div className="flex flex-col gap-6">
    {Array.from({ length: rows }).map((_, i) => (
      <div key={i} className="flex items-center gap-4">
        ...
        <Skeleton variant="text" width={60} height={10} className="shrink-0 opacity-40" />
      </div>
    ))}
  </div>
);
...
export const MetricSkeleton = ({ className }: { className?: string }) => (
  <div className={cn("glass flex flex-col gap-3 rounded-xl p-6 shadow-soft", className)}>
    <Skeleton variant="text" width="45%" height={12} className="opacity-70" />
    <Skeleton variant="text" width="80%" height={32} />
    <div className="flex items-center gap-2">
      <Skeleton variant="rectangular" width={40} height={16} className="rounded-full bg-primary/20" />
  ...
  <div className="w-full space-y-4">
    <div className="flex gap-4 border-b pb-4">
      {Array.from({ length: columns }).map((_, i) => (
         <Skeleton key={i} variant="text" width="15%" height={12} className="opacity-70" />
      ))}
    </div>
