...
describe("SpeedDial", () => {
  const mockActions = [
    { icon: <span data-testid="icon-1">1</span>, label: "Action 1", onClick: vi.fn() },
    { icon: <span data-testid="icon-2">2</span>, label: "Action 2", onClick: vi.fn() },
  ];
  ...
  it("applies correct direction classes", () => {
    const { rerender } = render(<SpeedDial actions={mockActions} direction="up" />);
    fireEvent.click(screen.getByRole("button"));
    let actionsContainer = screen.getByTestId("icon-1").closest('div')?.parentElement;
    expect(actionsContainer).toHaveClass("flex-col-reverse");

    rerender(<SpeedDial actions={mockActions} direction="right" />);
    actionsContainer = screen.getByTestId("icon-1").closest('div')?.parentElement;
